...
const btnPlayer1Attack = document.querySelector('.btnPlayer1Attack');
const btnPlayer2Attack = document.querySelector('.btnPlayer2Attack');
const player1HealthDisplay = document.querySelector('.player1Health');
const player2HealthDisplay = document.querySelector('.player2Health');


let currentPlayer = 1;


btnPlayer1Attack.addEventListener('click', () => {
    if (currentPlayer === 1) { 
        const damage = player1.atk;
        player2.pv -= damage;
        checkEndGame();
        player2HealthDisplay.textContent = `Point de vie du joueur 2 : ${player2.pv}`;
        currentPlayer = 2; 
        btnPlayer2Attack.disabled = false; 
        btnPlayer1Attack.disabled = true; 
    }
});

btnPlayer2Attack.addEventListener('click', () => {
    if (currentPlayer === 2) { 
        const damage = player2.atk;
        player1.pv -= damage;
        checkEndGame();
        player1HealthDisplay.textContent = `Point de vie du joueur 1 : ${player1.pv}`;
        currentPlayer = 1; 
        btnPlayer1Attack.disabled = false; 
        btnPlayer2Attack.disabled = true; 
    }
});
...
function checkEndGame() {
    if (player1.pv <= 0) {
        alert(`${player2Name} a gagné !`);
    } else if (player2.pv <= 0) {
        alert(`${player1Name} a gagné !`);
    }
}
